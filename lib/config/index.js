...
const CONFIG_PROCESS_INT_TIMEOUT = 10 * 1000;
const CONFIG_PROCESS_KILL_TIMEOUT = 14 * 1000;
const CONFIG_PROCESS_LONG_INTERVAL = 4 * 1000;
...
function configProcessMessageHandler(file, resolve, reject) {
    function warn(...args) {
        exports.emit('warn', file, args.join(' '));
    }

    // We store result and killed because we cannot reject early or else
    // we couldn't cleanly shut down the child.
    let result = null;
    let killed = 0;

    const intervalLong = setInterval(() => {
        warn("Still running, please wait...");
    }, CONFIG_PROCESS_LONG_INTERVAL);
    const timeoutINT = setTimeout(() => {
        warn("Timed out; sending SIGINT");
        killed = 1;
        this.kill('SIGINT');
    }, CONFIG_PROCESS_INT_TIMEOUT);
    const timeoutKILL = setTimeout(() => {
        warn("SIGINT ignored; sending SIGKILL so it has no choice");
        killed = 2;
        this.kill('SIGKILL');
    }, CONFIG_PROCESS_KILL_TIMEOUT);

    this.on('message', function messageListener(message) {
        if (result && message.error) {
            warn(`Error after producing a result: ${errorMessageToString(message)}`);
            return;
        }

        if (result) {
            warn("Somehow produced a second result; this is a BUG");
            return;
        }
    ...
    this.once('exit', (code, signal) => {
        ...
        if (killed === 2) {
            reject(`Didn't exit within ${CONFIG_PROCESS_KILL_TIMEOUT} ms and had to be killed`);
            return;
        }

        if (code === null && killed) {
            reject(`Didn't exit within ${CONFIG_PROCESS_INT_TIMEOUT} ms`);
            return;
        }
        ...
        if (result.error) {
            reject(`Raised an error: ${errorMessageToString(result)}`);
            return;
        }
