...
exports.getConfig = async function(options) {
    ...
    const globalConfig = Object.assign({}, await getGlobalConfig(args));
...
function getGlobalConfig() {
    ...
    return GLOBAL_CONFIG = (async () => {
        ...
        return await validateConfig(config, false, DEFAULT_CONFIG_PATH);
    })();
}
...
async function parseFullConfig(config) {
    ...
    if (socket.protocol === 'ws:') {
        ...
        if (os.platform() === 'win32') {
            throw new ConfigError("Unix domain sockets are unsupported on Windows platforms. Bind to a port instead.", config['config-path']);
        }
...
function configProcessMessageHandler(file, resolve, reject) {
    ...
    this.on('message', function messageListener(message) {
        ...
        result = {
            error: new ConfigError("Produced a second result; this is a BUG", file)
        };
    });
