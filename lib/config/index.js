...
exports.resolveConfig = async function resolveConfig(configPath) {
    ...
    try {
        ...
        const paths = (await Promise.all([
            resolveConfig(path.join(resolved, './process-config.js')),
            resolveConfig(path.join(resolved, './process-config.json'))
        ])).filter(_ => _);

        if (paths.length === 0 && checkParent) {
            return resolveConfig(path.join(configPath, '../'));
        }
