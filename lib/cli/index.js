...
exports.ACTIONS = {
    'start': require('./actions/start.js'),
    'restart': require('./actions/start.js'),
    'add': require('./actions/add.js'),
    'delete': require('./actions/delete.js'),
    'stop': require('./actions/stop.js'),
    'kill': require('./actions/stop.js'),
    'scale': require('./actions/scale.js'),
    'show': require('./actions/show.js'),
    'inspect': require('./actions/inspect.js'),
    'log': require('./actions/log.js'),
};

exports.args = null;

exports.bugs = require('./node-bugs.js');
...
exports.invalidConfig = function(path, ...info) {
    exports.err("Configuration", exports.escape(path), ...(info.map(exports.escape)));
...
exports.table = function(data, options = {}) {
    data = data.map((line) => {
        return line.map(logging.colorize);
    });
...
exports.filterInfo = function(info, selectors, options = {}) {
    const applications = info.applications.filter((app) => {
        for (const select of selectors) {
            ...
            if (select.app === app.name ||
                select.app === 'all' && (app.type !== 'logger' || options.implicitLoggers))
                return true;
        }
    ...
    function processFilter(process) {
        for (const select of selectors) {
            ...
            if (select.app && select.app === 'all' && app.type === 'logger' && !options.implicitLoggers) {
                continue;
            }

            const matchResults = [
                match('number', select, process, options),
                match('id', select, process, options),
                match('pid', select, process, options)
            ];

            const passes = matchResults.reduce((a, b) => options.exact ? a || b : a && b);
    ...
    function match(name, select, process, options) {
        if (typeof select[name] === 'undefined') {
            return !options.exact;
        }
        return select[name] === process[name];
    }
