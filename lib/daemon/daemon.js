...
class Daemon extends EventEmitter {
    ...
    logs(appName, options, client) {
        ...
        if (appName === 'all') {
            loggers = this.loggers.values();
        } else {
            loggers = new Set(
                this.processes.values()
                    .filter((process) => process.app['name'] === appName)
                    .map((process) => this.getLogger(process))
            );
        }

        const lines = loggers.values()
            .reduce((lines, logger) => lines.concat(logger.logs.get(appName).lines), [])
            .map((line) => line.data)
...
function processEvents(daemon) {
    return {
        ...
        exit(code) {
            ...
            if (this.app['type'] === 'logger' && code) {
                daemon.err(this, 'Logger crashed:');

                const logger = daemon.getLogger(this);
                const logs = logger.logs.get(this.app['name']);

                for (let line of logs.lines) {
                    if (line.process !== this)
                        continue;

                    daemon.err(this, '   ', line.data.text);
                }
            }
        }
    };
