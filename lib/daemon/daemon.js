...
class Daemon extends EventEmitter {
    constructor() {
        ...
        this.daemonLogger = new DaemonLogger(this);
...
class DaemonLogger {
    ...
    log(process, type, text) {
        this.daemon.log('Logger', process, type, text);
    }

    pushLine(process, stream, line) {
        this.log(process, stream, line);
    }
...
const loggerEventCache = new WeakMap();
function createLoggerEvents(logger) {
    if (loggerEventCache.has(logger)) {
        return loggerEventCache.get(logger);
    }
    ...
    loggerEventCache.set(logger, events);

    return events;
}

function attachLogger(process, logger) {
    const events = createLoggerEvents(logger);
