...
class Logger extends events.EventEmitter {
    ...
    flushAsync(appName) {
        setImmediate(() => this.flush(appName));
    }
    ...
    flush(appName) {
        ...
        if (!this.currentProcess) {
            if (logs.writeIndex < logs.lines.length &&
                appName !== this.name && this.processes.size === 0) {
    ...
    flushAll() {
        ...
        for (const app of this.logs.keys()) {
            this.flush(app);
        }
    }
...
function processEvents(logger) {
    return {
        ...
        exit(code, signal) {
            ...
            logger._pushLogEvent(logs, this, 'exit', `code=${code} signal=${signal}`);

            if (this !== logger.currentProcess) {
                logger.flushAsync(app['name']);
            }
        },
        output(stream, data) {
