...
const APIDefinition = {
    ...
    delete: [
        applicationSchema.extract('name').required()
    ],
    ...
    follow: [APPLICATION_NAME_OR_ALL],

    unfollow: [APPLICATION_NAME_OR_ALL],

    killDaemon: [],

    all(args) {
        const result = API_CALL_ARRAY.validate(args[0], { presence: 'required' });
...
function runDefinition(definition, args) {
    ...
    if (Joi.isSchema(definition)) {
        const {error} = definition.validate(args, { presence: 'required' });
